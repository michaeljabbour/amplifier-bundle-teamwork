"""Runner for eval 03: can a single-window judge separate RECORD from SKIP for a LESSON?

The question this spike answers, and the only one (see
docs/scenarios/06-record-the-lesson-not-the-incident.md and its twin
docs/scenarios/06b-the-lesson-that-is-only-true-on-your-machine.md): can a
model, shown ONE window with no comparison available, separate a lesson whose
evidence reaches as far as its claim from one that is real, correctly
learned, and still bounded to one environment? If it cannot, no amount of
hook engineering, statefulness, or windowing helps, and `detect_lessons`
should not be shipped on -- a report saying so is a success.

GRADING LIMIT, stated plainly per 06's own open question: 06 names that a
rubric derived from it cannot score a single run -- only a PAIR, a run that
records and a later run that either benefits from it or repeats the mistake.
This eval does NOT attempt to score that pair. It answers the narrower,
necessary-but-not-sufficient question: can the judge classify one window
correctly at all? A perfect score here does not establish that a later
session actually benefits from what gets recorded; see this eval's own
README for that limit spelled out.

FIDELITY RULE, non-negotiable, matching evals/02-decision-detection/run.py:
each of the six cases in cases.py goes to its own isolated
`decision_judge.judge_lesson_window()` call. A judge that sees several at
once can contrast them, which makes the task easier than reality and
...
`judge_lesson_window()` caller in production would hand it. It carries no
...
recall here -- a missed lesson costs a re-discovery someone can make again,
...
async def run():
    ...
    print("Lesson-detection spike -- %d/%d correct" % (correct, len(results)))
