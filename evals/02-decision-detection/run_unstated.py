"""Runner for the unstated-reason case set: the hard half of eval 02.

Same fidelity rule as run.py: each of the six cases in cases_unstated.py goes
to its own isolated `decision_judge.judge_window()` call, with no shared
memory between cases and one real top-level session built once purely so
each isolated call has a provider to inherit from.

Per the task that asked for this set: run it live and report the real
per-case results and the false-positive count SEPARATELY from run.py's
already-passing self-explaining set. If precision collapses here, that is
the honest, plainly-stated finding -- not a reason to keep tuning the prompt
until it passes.
...
results/ (gitignored) with an `_unstated` suffix so it is never confused with
run.py's own reports.
"""
...
from cases_unstated import CASES_UNSTATED
...
def assess_case(case, verdict):
    """Unavailable output is neither a successful SKIP nor a missed decision."""
...
async def run():
    ...
    report = {
        ...
        "model": JUDGE_MODEL,
        "case_set": "unstated-reason (hard case, see 08b's open questions)",
        "correct": correct,
        "total": len(results),
    ...
    out_path = out_dir / (
        datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ") + "_unstated.json"
    )
    out_path.write_text(json.dumps(report, indent=2), encoding="utf-8")

    print("Decision-detection spike (UNSTATED-REASON set) -- %d/%d correct" % (correct, len(results)))
