...
3. **If you have run `az login`** (Entra sign-in via the Azure CLI), the form offers Microsoft sign-in: leave the member-code fields blank and confirm sharing. When this directory has a usable GitHub remote, the form shows the exact repository URL that will be sent and the project ID becomes optional -- Teamwork resolves the project from that repository. Otherwise (or if sign-in does not match a Teamwork member), enter the exact project ID you joined, your name/email and private member code, and confirm sharing. Never paste the code into chat. On later sessions, select the same project and consent again; leave login fields blank to reuse its saved connection.
...
see. If neither resolves anything, pass `project_id` explicitly.
