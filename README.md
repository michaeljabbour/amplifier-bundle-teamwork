...
3. The form supports three enrollment methods, tried in this order for whatever you fill in and submit:
   ...
   2. **Microsoft sign-in (`az login`), when the service advertises it.** If you have run `az login` and left both the credential and member-code fields blank, the form offers Microsoft sign-in. When this directory has a usable GitHub remote, the form shows the exact repository URL that will be sent and the project ID becomes optional -- Teamwork resolves the project from that repository. **The service now advertises `api_app_id` via `/api/config`** -- measured 2026-09-17 against the canonical deployment, from the host and again from inside a container, returning a non-empty `api_app_id`, `tenant_id`, `auth_mode: "sso"` and `project_id`. A client on a host with `az login` acquires a correctly-audienced token for it (the token's `aud` and `tid` match what `/api/config` advertises, verified by decoding the issued JWT). **The other half has now been measured too, and it FAILS.** Driving this bundle's own `_mint_sso` against the live deployment on 2026-09-17 returned `403 not_a_member` -- *"Microsoft sign-in did not match a Teamwork member"* -- for an identity whose member-code enrollment succeeded minutes earlier in the same session. The token validates (a validation failure raises `401` instead); the identity is simply absent from the service's roster map, which is built from each member's `email` and `aliases`. Until a maintainer adds those, **expect Microsoft sign-in to be refused and the form to fall back to the member-code fields** -- which ARE proven: an enrollment against the live service succeeded on 2026-09-17 and wrote a credential.
